...
export default Ember.Service.extend({
    ...
    // These are set by mayasecure/route
    ...
    testAuth() {
    // make a call to api base because it is mayasecure
    ...
    setOrganization(){
        this.get('userStore').find('organizations', null, {url: 'organization'}).then((res) => {
            if ( res ){
                this.get('session').set('currentOrgId',res.content[0].id);
                this.get('session').set('currentOrgState',res.content[0].state); 
            }
        }).catch((/*res*/)=>{
    ...
    login(code) {
        ...
        return this.get('userStore').rawRequest({
            ...
            data: {
                code: code,
                authProvider: this.get('session').get('authProvider'),
                inviteCode: this.get('session').get('invitecode'),
            },
        }).then((xhr) => {
            var auth = xhr.body;
            var interesting = {};
